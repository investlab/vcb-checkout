...
exportData.set = function (title, export_url) {
    ...
    exportData.showMessageWarning('Đang xử lý ...');
...
exportData._getData = function (url) {
    if (exportData.isStop == false) {
        exportData.url = url;console.log(url);
        $.get(url, function (data) {
            if (data) {
                var result = JSON.parse(data);console.log(result);
                if (result.error == '') {
                    if (result.type_url == 'get_data') {
                        exportData.showMessageWarning('Đang lấy dữ liệu (đã xử lý được ' + result.row_processed + ' dòng) ...');
                        window.setTimeout('exportData._getData("' + result.next_url + '");', 500);
                    } else {
                        exportData.showMessageWarning('Đang xuất file excel ...');
...
exportData._checkIframeLoaded = function () {
    if ($('#iframe-export-data').length && $('#iframe-export-data').attr('readyState') == 'complete') {
        exportData.showMessageSuccess('Đã xuất file excel');
...
exportData.showMessageWarning = function (message) {
    var html = '<div class="alert alert-warning"><i class="glyphicon glyphicon-download-alt color-dark"></i>  ' + message + '</div>';
    $('#modal-export-data .modal-body').html(html);
};

exportData.showMessageSuccess = function (message) {
    var html = '<div class="alert alert-success"><i class="glyphicon glyphicon-ok"></i>  ' + message + '</div>';
    $('#modal-export-data .modal-body').html(html);
    if ($('#iframe-export-data').length) {
        $('#iframe-export-data').remove();
    }
