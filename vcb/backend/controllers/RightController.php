...
use backend\models\form\RightListForm;
...
class RightController extends BackendController
{
    ...
    public function actionActive()
    {
        $message = '';
        //---------
        $model = RightUpdateForm::findOne(['id' => Yii::$app->request->get('id'), 'status' => Right::STATUS_LOCK]);
        if ($model == null) {
        ...
        $url = Yii::$app->urlManager->createAbsoluteUrl('right/index', HTTP_CODE);
    ...
    public function actionAdd()
    {
        ...
        return $this->render('add', [
            ...
            'parent_id_array' => Right::getCategories(array(0 => '----'. Translate::get('Danh mục gốc').' ----')),
        ]);
    }
    ...
    public function actionDelete()
    {
        $message = '';
        //---------
        $model = RightDeleteForm::findOne(['id' => Yii::$app->request->get('id')]);
        if ($model == null) {
            $message = Translate::get('Tham số đầu vào không hợp lệ, truy cập bị từ chối');
        } else {
            if ($model->delete()) {
    ...
    {
        $model = new RightListForm();
        $model->load(Yii::$app->request->get(), 'RightListForm');
    ...
    public function actionLock()
    {
        $message = '';
        //---------
        $model = RightUpdateForm::findOne(['id' => Yii::$app->request->get('id'), 'status' => Right::STATUS_ACTIVE]);
        if ($model == null) {
            $message = Translate::get('Tham số đầu vào không hợp lệ, truy cập bị từ chối');
        } else {
