            ...
            <div class="option-buttons">
                <div class="addNew">
                    <a class="btn btn-danger btn-sm"
                       href="<?= Yii::$app->urlManager->createUrl('checkout-order-backup/index') ?>">
    ...
    <div class=panel-body>
        <table class="table table-hover" width="100%">
            ...
            <tr>
                ...
                <td>
                    <?php if ($checkout_order['status'] == CheckoutOrderBackup::STATUS_NEW) { ?>
                        <span class="label label-default"><?= Translate::get('Chưa thanh toán') ?></span>
                    <?php } elseif ($checkout_order['status'] == CheckoutOrderBackup::STATUS_PAYING) { ?>
                        <span class="label label-warning"><?= Translate::get('Đang thanh toán') ?></span>
                    <?php } elseif ($checkout_order['status'] == CheckoutOrderBackup::STATUS_PAID) { ?>
                        <span class="label label-success"><?= Translate::get('Đã thanh toán') ?></span>
                    <?php } elseif ($checkout_order['status'] == CheckoutOrderBackup::STATUS_CANCEL) { ?>
                        <span class="label label-danger"><?= Translate::get('Đã hủy') ?></span>
                    <?php } elseif ($checkout_order['status'] == CheckoutOrderBackup::STATUS_REVIEW) { ?>
                        <span class="label label-danger"><?= Translate::get('Bị review') ?></span>
                    <?php } elseif ($checkout_order['status'] == CheckoutOrderBackup::STATUS_WAIT_REFUND) { ?>
                        <span class="label label-warning"><?= Translate::get('Đang đợi hoàn tiền') ?></span>
                    <?php } elseif ($checkout_order['status'] == CheckoutOrderBackup::STATUS_REFUND) { ?>
                        <span class="label label-success"><?= Translate::get('Đã hoàn tiền') ?></span>
                    <?php } elseif ($checkout_order['status'] == CheckoutOrderBackup::STATUS_WAIT_WIDTHDAW) { ?>
                        <span class="label label-warning"><?= Translate::get('Đang rút tiền') ?></span>
                    <?php } elseif ($checkout_order['status'] == CheckoutOrderBackup::STATUS_WIDTHDAW) { ?>
            ...
            <tr>                
                ...
                <td>
                    <?php if ($checkout_order['callback_status'] == CheckoutOrderBackup::CALLBACK_STATUS_NEW) { ?>
                        <span class="label label-default"><?= Translate::get('Chưa gọi') ?></span>
                    <?php } elseif ($checkout_order['callback_status'] == CheckoutOrderBackup::CALLBACK_STATUS_PROCESSING) { ?>
                        <span class="label label-warning"><?= Translate::get('Đang gọi') ?></span>
                    <?php } elseif ($checkout_order['callback_status'] == CheckoutOrderBackup::CALLBACK_STATUS_SUCCESS) { ?>
                        <span class="label label-success"><?= Translate::get('Đã gọi') ?></span>
                    <?php } elseif ($checkout_order['callback_status'] == CheckoutOrderBackup::CALLBACK_STATUS_ERROR) { ?>
...
<div class="col-lg-12" style="padding:20px 0px 30px">
    <a href="<?= Yii::$app->urlManager->createUrl('checkout-order-backup/index') ?>" class="btn btn-danger btn-sm">
        <i class="en-back"></i> <?= Translate::get('Quay lại') ?>
    </a>
</div>
</div>
