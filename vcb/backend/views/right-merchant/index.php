...
/* @var $form ActiveForm */
$this->title = Translate::get('Danh sách quyền tài khoản merchant');
        ...
        <div class="col-lg-12 heading">
            <div id="page-heading" class="heading-fixed">
                ...
                <div class="option-buttons">
                    <div class="addNew"><a href="<?= Yii::$app->urlManager->createUrl(['right-merchant/add'])?>" data-toggle="modal" class="btn btn-sm btn-success">
<i class="en-plus3"></i> <?= Translate::get('Thêm') ?></a></div>
    ...
    <div class=outlet>
        <div class="well well-sm fillter">
            <?php $form = ActiveForm::begin(['method' => 'get', 'action' => Yii::$app->urlManager->createUrl(['right-merchant']), 'enableAjaxValidation' => true, 'options' => ['class' => 'form-horizontal', 'role' => 'form']]); ?>
            <div class="row group-input-search">
                ...
                <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3 no-padding-left group-btn-search mobile-flex-middle-center">
                    <?= Html::submitButton(Translate::get('Tìm kiếm'), ['class' => 'btn btn-danger']) ?>&nbsp;&nbsp;
                    <a class="btn btn-default" href="<?= Yii::$app->urlManager->createUrl('right-merchant') ?>"><?= Translate::get('Bỏ lọc') ?></a>
                </div>
            </div>
            <?php ActiveForm::end(); ?>
        </div>
        <div class="table-responsive">
        <form method="post" action="<?= Yii::$app->urlManager->createUrl(['right-merchant/update-position'])?>">
            <table class="table table-bordered" border="0" cellpadding="0" cellspacing="0" width="100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center"><span class="<?= $row["status_class"] ?>"><?= $row["status_name"] ?></span></td>
                        <td>
                            ...
                            <div class="dropdown otherOptions fr">
                                ...
                                <ul class="dropdown-menu right" role="menu">
                                    ...
                                    ?>
                                        <li><a href="<?= Yii::$app->urlManager->createUrl(['right-merchant/' . $key, 'id' => $row['id']]) ?>" onclick="
                                                confirm('<?= $operator['title'] ?> <?=Translate::get('quyền hệ thống')?>', '<?= Yii::$app->urlManager->createUrl(['right-merchant/' . $key, 'id' => $row['id']]) ?>');return false;"><?= $operator['title'] ?></a></li>
                                    <?php else:?>
                                        <li><a href="<?= Yii::$app->urlManager->createUrl(['right-merchant/' . $key, 'id' => $row['id']]) ?>"><?= $operator['title'] ?></a></li>
                                    <?php 
                                        endif;
                                    endforeach; 
                                    ?>    
