...
$fee = $model->getFeeByCardTypeAndCycle($model->payment_method_code, $customer_field['card_type'], $customer_field['cycle']) != null ? $model->getFeeByCardTypeAndCycle($model->payment_method_code, $customer_field['card_type'], $customer_field['cycle']) : '';
...
<div class="row">
    ...
    <div class="form-horizontal" id="id-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-7" id="cdtg-7">
                <ul class="cardList clearfix">
                    <li>
                        <div class="radio-toolbar boxWrap card-bank" id="card-bank" style="">
                            <input type="hidden" name="card_info" value="<?php echo $card_type[$customer_field['card_type']] ?>" id="<?= $customer_field['card_type'] ?>"
                                   class="card_info">
                            <i class="<?= $customer_field['card_type'] ?> card-bank-i" style="top: 0"></i>
                        </div>
                    </li>
                </ul>
                <?= $form->field($model, 'card_info')->input('text', array('class' => 'hidden', 'id' => 'card_info', 'value' => ""))->label(false); ?>
                <input type="hidden" name="card_type" value="<?= $customer_field['card_type'] ?>" class="card_type" id="card_type">
        ...
        <div class="form-group form-cycle" id="form-cycle">
            ...
            <div class="col-sm-7" style="display: block; ">
                <div style="display:flex">
                    <div class="radio-toolbar list-cycle list-cycle-<?php echo $customer_field['card_type']; ?>"
                         id="kyhan" style="">
                        <input id="card_type_sel" type="radio"
                               name="card_cycle" value="<?= $customer_field['cycle'] ?>" class="cycle"
                               onclick="getCycle(<?= $fee ?>,<?= $customer_field['cycle'] ?>)">
                        <label for="card_type_sel"
                               style="margin-bottom: 0px"><?= $customer_field['cycle'] . ' ' . Translate::get('Tháng') ?></label>
                    </div>
    ...
    <script>
        ...
        function getCycle(fee, cycle) {
            ...
            if (card_fee_bearer == 3) {
                ...
                amount_fee = amount_order + (amount_order * fee) + partner_payment_fee;
        ...
        $("#card_type_sel").click()
        $("#card-bank").click()
        getCycle(<?= $fee ?>,<?= $customer_field['cycle'] ?>)
