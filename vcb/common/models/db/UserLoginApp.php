 ...
 */
class UserLoginApp extends MyActiveRecord implements IdentityInterface {

    const STATUS_ACTIVE = 1;
    const STATUS_LOCK = 2;
    const MALE = 1;
    const FEMALE = 2;
    const TOKEN_STATUS_ACTIVE = 0;
    const TOKEN_STATUS_LOCK = 1;
            ...
            if ($user_login_info != false && $user_login_info->status == self::STATUS_ACTIVE) {
                if (self::_checkIpLimitation($user_login_info->ips, $user_ip)) {
                    if ($user_login_info->password == self::encryptPassword($password)) {
                        ...
                        self::checkLoginFail($user_login_info->id);
    ...
    public static function findIdentityByAccessToken($token, $type = null) {
        ...
        Yii::warning('findIdentityByAccessToken: ' . $token . ' | ' . $type);

        $deviceToken = Yii::$app->request->headers->get('device-token');
        //$result_session = Yii::$app->session->get('user_info_' . md5($deviceToken));

        //Yii::info('findIdentityByAccessToken: ' . $token);
        $result = \app\models\bussiness\UserFlow::getUserByAccessToken(['access_token' => $token]);
        //var_dump($result['response']);
        //token_status = 2 : hết hạn , token_status = 1 : còn hạn
        if ($result['error_code'] == '0') {
            if ($result['response']['status'] == 0){
                Yii::info('$deviceToken: ' . $deviceToken . '|' . $result['response']['device_token'] . '|' . $result['response']['status']);

                if ($result['response']['device_token'] != $deviceToken) {

                    throw new UnauthorizedHttpException('Tài khoản đã được đăng nhập ở thiết bị khác  Vui lòng đăng nhập lại.', 10001);
                }

                if (@$result['response']['user_info']['status'] == self::STATUS_ACTIVE) {
                    $result['response']['user_info']['client_ip'] = $result['response']['clientIp'];
                    $result['response']['user_info']['device_token'] =  $result['response']['device_token'];
                    $result['response']['user_info']['access_token'] =  $result['response']['access_token'];
                    $result['response']['user_info']['time_expired'] =  $result['response']['timeLimit'];
                    return new static($result['response']['user_info']);
                } else
                    throw new UnauthorizedHttpException('Tài khoản của bạn bị khóa');
            }else{
                throw new UnauthorizedHttpException('Phiên làm việc của bạn hết hạn. Vui lòng đăng nhập lại.', 10001);
