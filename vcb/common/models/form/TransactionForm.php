...
class TransactionForm extends LanguageBasicForm
{
    ...
    {
        return [
            [['amount','bank_refer_code'], 'required', 'message' => 'Bạn phải nhập {attribute}'],
            [['merchant_id', 'payment_method_id','partner_payment_id'], 'number', 'min' => 1, 'tooSmall' => 'Bạn phải chọn {attribute}.'],
            [['id', 'bank_card_month', 'bank_card_year','zone_id'], 'integer', 'message' => '{attribute} không hợp lệ.'],
            [['bank_account_code', 'bank_account_name', 'bank_account_branch','bank_refer_code'], 'string'],
    ...
    public function attributeLabels()
    {
        return [
            'id' => 'ID',
            'payment_method_id' => 'Phương thức nạp',
            ...
            'partner_payment_id'=>'Kênh nạp',
            'bank_refer_code' => 'Mã tham chiếu',
        ];
    }
    ...
    public function checkValidate($attribute, $param)
    {
        switch ($attribute) {
            case "time_end":
                ...
                break;
            case "amount":
                if (ObjInput::formatCurrencyNumber($this->amount) < 2000 ) {
                    $this->addError('amount', 'Số tiền nạp phải lớn hơn 2000 VND');
                }
                break;
