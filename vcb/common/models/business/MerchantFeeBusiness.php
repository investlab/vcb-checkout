...
class MerchantFeeBusiness
{
    ...
    static function addAndActive($params, $rollback = true)
    {
        ...
        $id = null;
        if ($rollback) {
            $transaction = MerchantFee::getDb()->beginTransaction();
        }
    ...
    static function add($params, $rollback = true)
    {
        ...
        if (self::_addValidate($params, $error_message, $data)) {
            ...
            $model->time_end = 0;
            $model->status = MerchantFee::STATUS_NEW;
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    $error_message = 'Có lỗi khi thêm phí merchant';
                }
            } else {
    ...
    /**
     *
     * @param type $params : merchant_fee_id, user_id
     * @param type $rollback
     * @return type
     */
    static function request($params, $rollback = true)
    {
        ...
        //----------
        $model = MerchantFee::findOne(['id' => $params['merchant_fee_id'], 'status' => MerchantFee::STATUS_NEW]);
        if ($model != null) {
            $model->status = MerchantFee::STATUS_REQUEST;
            $model->time_updated = time();
            ...
            $error_message = 'Phí merchant muốn kích hoạt không tồn tại';
        }
        if ($rollback) {
            if ($commit == true) {
                $transaction->commit();
    ...
    static function active($params, $rollback = true)
    {
        ...
        //----------
        $model = MerchantFee::findOne(['id' => $params['merchant_fee_id'], 'status' => MerchantFee::STATUS_REQUEST]);
        if ($model != null) {
            $model->status = MerchantFee::STATUS_ACTIVE;
            $model->time_active = time();
            $model->user_active = $params['user_id'];
            if ($model->validate() && $model->save()) {
                $inputs = array(
                    'merchant_fee_id' => $params['merchant_fee_id'],
                    'user_id' => $params['user_id'],
                );
            ...
            $error_message = 'Cấu hình phí thanh toán muốn kích hoạt không tồn tại';
        }
        if ($rollback) {
            if ($commit == true) {
                $transaction->commit();
    ...
    static function lock($params, $rollback = true)
    {
        ...
        //----------
        $model = MerchantFee::findOne(['id' => $params['merchant_fee_id'], 'status' => MerchantFee::STATUS_ACTIVE]);
        if ($model != null) {
            $model->status = MerchantFee::STATUS_LOCK;
            ...
            $error_message = 'Phí merchant muốn khóa không tồn tại';
        }
        if ($rollback) {
            if ($commit == true) {
                $transaction->commit();
    ...
    private static function _updateTimeEndForActive($params, $rollback = true)
    {
        ...
        if ($model != null) {
            $all = false;
            $previous = Tables::selectOneDataTable("merchant_fee", "id != " . $params['merchant_fee_id'] . " "
                . "AND merchant_id = " . $model->merchant_id . " AND method_id = " . $model->method_id . " AND payment_method_id = " . $model->payment_method_id . " AND min_amount = " . $model->min_amount . " AND status = " . MerchantFee::STATUS_ACTIVE . " "
            ...
            if ($previous != false) {
                $model_previous = MerchantFee::findOne(['id' => $previous['id'], 'status' => MerchantFee::STATUS_ACTIVE]);
            ...
            if ($all == true) {
                $all = false;
                $next = Tables::selectOneDataTable("merchant_fee", "id != " . $params['merchant_fee_id'] . " "
                    . "AND merchant_id = " . $model->merchant_id . " AND method_id = " . $model->method_id . " AND payment_method_id = " . $model->payment_method_id . " AND min_amount = " . $model->min_amount . " AND status = " . MerchantFee::STATUS_ACTIVE . " "
            ...
            $error_message = 'Cấu hình phí thanh toán muốn cập nhật không tồn tại';
        }
        if ($rollback) {
            if ($commit == true) {
                $transaction->commit();
    ...
    private static function _updateTimeEndForLock($params, $rollback = true)
    {
        ...
        //----------
        $model = MerchantFee::findOne(['id' => $params['merchant_fee_id'], 'status' => MerchantFee::STATUS_LOCK]);
        if ($model != null) {
            $all = false;
            $previous = Tables::selectOneDataTable("merchant_fee", "id != " . $params['merchant_fee_id'] . " "
                . "AND merchant_id = " . $model->merchant_id . " AND method_id = " . $model->method_id . " AND payment_method_id = " . $model->payment_method_id . " AND min_amount = " . $model->min_amount . " AND status = " . MerchantFee::STATUS_ACTIVE . " "
                . "AND (time_begin < " . $model->time_begin . " OR (time_begin = " . $model->time_begin . " AND time_active < " . intval($model->time_active) . " ))", "time_begin DESC, time_active DESC ");
            if ($previous != false) {
                $model_previous = MerchantFee::findOne(['id' => $previous['id'], 'status' => MerchantFee::STATUS_ACTIVE]);
                if ($model_previous != null) {
                    $next = Tables::selectOneDataTable("merchant_fee", "id != " . $params['merchant_fee_id'] . " "
                        . "AND merchant_id = " . $model->merchant_id . " AND method_id = " . $model->method_id . " AND payment_method_id = " . $model->payment_method_id . " AND min_amount = " . $model->min_amount . " AND status = " . MerchantFee::STATUS_ACTIVE . " "
                        . "AND (time_begin > " . $model->time_begin . " OR (time_begin = " . $model->time_begin . " AND time_active > " . intval($model->time_active) . " ))", "time_begin ASC, time_active ASC ");
                    if ($next != false) {
            ...
            $error_message = 'Phí merchant muốn cập nhật không tồn tại';
        }
        if ($rollback) {
            if ($commit == true) {
                $transaction->commit();
