...
class CyberSourceVcb extends \SoapClient {
    ...
    private function getConnectParams() {
        ...
        if (!empty($partner_payment_account_info)) {
            $this->partner_payment_account_info = $partner_payment_account_info;
    ...
    public function authorizeCard3Dsecure($params) {
        $order= CheckoutOrder::find()->select(['order_description'])->where(['order_code'=>$params['order_code'],'merchant_id'=>23])->one();
        if ($order!=null){
            $description=$order->order_description;
            $ccAuthService= array(
            'run' => 'true',
            'reconciliationID'=>$description
        );
        }else {
            $ccAuthService = array(
                'run' => 'true',
...
//        }
    ...
    private function _writeLog($data, $breakLine = true, $addTime = true) {
        $file_name = 'cbs_vcb/output/' . date('Ymd') . '.txt';
