...
class PartnerPaymentMsbVa extends PartnerPaymentBasic
{
    const BANK_ID = "MSB";
    ...
    public function processRequest(PaymentMethodBasicForm &$form, $params)
    {
        ...
        $result = MSBVA::createVA($data_input, $form, $params);
    ...
    protected function _getAuthenUrl(&$result, PaymentMethodBasicForm $form, $transaction_id, $total_amount): string
    {
        ...
        $gen = Generator::create()->bankId(self::BANK_ID) // BankId, bankname
                ->accountNo($result['data']['rows'][0]['accountNumber'])// Account number
//                ->accountNo('19001010588421')// số tài khoản của QuangNT test :)))
                ->amount($total_amount)// Money
                ->info("VCBPG". $transaction_id) // Ref
                ->returnText(false) // if true, return text. If false, return image in base64
                ->logoPath(ROOT_PATH . DS . 'checkout/web/bank/center_qr/vietqr.png')
                ->generate();
