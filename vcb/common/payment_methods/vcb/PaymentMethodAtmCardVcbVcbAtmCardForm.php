...
class PaymentMethodAtmCardVcbVcbAtmCardForm extends PaymentMethodAtmCardForm {
    ...
    public function attributeLabels() {
        return [
            ...
            'card_number' => Translate::get('Số thẻ ATM'),
            'card_month' => Translate::get('Tháng trên thẻ'),
            'card_year' => Translate::get('Năm trên thẻ'),
            'verifyCode' => Translate::get('Mã bảo mật'),
            'otp' => Translate::get('Mã xác thực OTP'),
        ];
    }
    ...
    function initRequest(PartnerPaymentBasic &$partner_payment) {
        ...
        if ($fields !== false) {
            ...
            $this->error_message = Translate::get('Có lỗi kết nối tới ngân hàng, vui lòng chọn hình thức thanh toán khác');
        }
    ...
    function initVerify(PartnerPaymentBasic &$partner_payment) {
        $fields = $partner_payment->initVerify($this);
        if ($fields !== false) {
            if (!empty($fields)) {
                $this->_setFieldsRequire($fields);
            } else {
        ...
        switch ($field_code) {
            case 'BANK_ACCOUNT':
                return array(
                    'name' => 'card_number', 
                    'rules' => array(
                        array('required', 'message' => Translate::get('Quý khách vui lòng nhập {attribute}.')),
                        array('isCardNumber'),
                    )
                );
            case 'BANK_NAME':
            ...
            case 'ISSUE_MONTH':
                return array(
                    'name' => 'card_month', 
                    'rules' => array(
                        array('required', 'message' => Translate::get('Quý khách vui lòng chọn {attribute}.')),
    ...
    public function isIssueCardMonth($attribute, $params) {
        ...
        if (!array_key_exists($value, $this->getCardMonths())) {
            $this->addError($attribute, Translate::get('Tháng phát hành thẻ không hợp lệ'));
        } else {
    ...
    public function isIssueCardYear($attribute, $params) {
        ...
        if (!array_key_exists($value, $this->getIssueCardYears())) {
            $this->addError($attribute, Translate::get('Năm phát hành thẻ không hợp lệ'));
        }
    }
    ...
    public function isOTP($attribute, $params) {
        ...
        if (!preg_match('/^[a-zA-Z0-9]{6,8}$/', $value)) {
            $this->addError($attribute, Translate::get('Mã xác thực OTP không hợp lệ'));
            return false;
        }
    }
        ...
        if ($result['error_message'] == '') {
            ...
            if ($transaction_info != false) {
                ...
                if ($result['error_message'] == '') {
                    ...
                    $inputs = array(
                        'transaction_id' => $transaction_id,
                        'partner_payment_method_refer_code' => $result['response']['hash_code'],
